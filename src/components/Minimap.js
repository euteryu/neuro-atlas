...
export function Minimap({ cameraInfo, view, onGalaxyClick }) {
  const MAP_SIZE = 200;
  const MAP_SCALE = 15; // A tighter scale for our smaller universe
  ...
  // --- THIS IS THE MAJOR LOGIC FIX ---
  let cameraPositionOnMap;
  if (view.mode === 'hub') {
    // In hub view, use the camera's actual world position.
    cameraPositionOnMap = cameraInfo.position;
  } else {
    // INSIDE a galaxy, lock the camera icon to that galaxy's position on the map.
  ...
  return (
    ...
    >
      <svg width={MAP_SIZE} height={MAP_SIZE} viewBox={`0 0 ${MAP_SIZE} ${MAP_SIZE}`}>
        ...
        {galaxyDots.map(dot => (
          <g key={dot.id} onClick={() => onGalaxyClick(dot.id)} className="cursor-pointer">
